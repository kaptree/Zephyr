...
Bug8:在归档查询的板块，应该设置默认是时间轴，但是时间轴的格式不能看到详细信息以及反馈信息，还有任务创建人信息以及任务创建人的信息，需要进行修复，方便用户查看。

- ✅ 已完成（2026-08-21）
- 根因：归档页视图默认值为卡片（`viewMode` 初始 `'card'`）；时间轴条目只渲染标题 + 两行内容预览 + 标签，未展示任务创建人、反馈信息，详细信息也被截断
- 修复（ArchivePage.vue）：
  - 默认视图改为**时间轴**（`viewMode` 初始 `'timeline'`，刷新后保持）
  - 时间轴条目卡片增强：
    - **任务创建人信息**：👤 人名（`creator?.name || owner?.name`）+ 归档日期
    - **详细信息**：任务内容预览由 2 行提升为 3 行（line-clamp-3），点击卡片仍可打开完整详情侧滑面板
    - **反馈信息**：新增 `feedbackList()` 汇总被指派人反馈（`assignees.feedback_content`），有反馈显示绿色「💬 人名：内容」区块，无反馈显示「暂无反馈」
- 验证：浏览器实测（默认选中「时间轴」且渲染时间轴样式、刷新后保持；条目显示创建人「张局长」+归档日期、内容 3 行预览、有反馈任务显示绿色「💬 张三：报表已汇总…」/无反馈显示「暂无反馈」；详情侧滑面板与卡片视图切换无回归；控制台无报错）；`npm run build` 通过
