...
> **拼音搜索**：全站所有搜索框均支持拼音匹配——输入汉字、拼音全拼或拼音首字母（如 `zs` → 张三、`jj` → 紧急、`qfxz` → 清风小组）均可命中，忽略大小写；覆盖任务/归档、模板、专项工作组、Bug 反馈、操作日志、工作报告，以及人员选择、聊天通讯录、标签选择等全部搜索场景。
...
- **无音频提醒**：未读时页面标题闪烁 + favicon 未读红点，页面在后台时通过系统级通知（Notification API）提醒，不依赖音频设备
...
- **仿 QQ/微信在线聊天**：会话/通讯录双 Tab（在线状态实时显示、未读角标），气泡消息左右分栏，聊天记录落库，WebSocket 实时推送
- **表情包 + emoji**：9 类 emoji 面板；内置熊猫头/狗狗/猫猫三套表情包，用户可上传自定义表情（「我的表情」），管理员支持文件夹/多文件批量上传；悬停表情放大预览
- **文件传输**：支持 ≤10MB 图片/文档/压缩包等绝大多数格式，图片消息直接预览（点击放大），其他文件以卡片展示可下载；文件黑白名单由管理员配置、保存即时生效
- **消息提醒**：新消息右上角丝滑动画弹窗（离线登录后自动补推），点击直达会话
...
- **通知中心**: `/api/v1/notifications/*`（列表/未读数/全部已读/单条删除）
- **即时聊天**: `/api/v1/chat/*`（会话/在线状态/消息/聊天附件），`/api/v1/emoticons/*`（表情包管理，管理员批量上传 `/emoticons/batch`）
