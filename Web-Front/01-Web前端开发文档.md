...
> 核心载体：桌面智能任务协同办公系统的Web管理端与即时协同大屏端

---

## 一、项目概述

Web前端作为系统的管理中枢与可视化展示端，承接桌面端任务的云端归档、跨终端检索、组织人员管理、模板配置及应急大屏协同等功能。页面以**纯白为底、极简为形、高级交互为魂**，打造零学习成本的公司办公新体验。
...
- 任务卡片：`0 4px 24px -4px rgba(0,0,0,0.08)`，hover时上浮 `translateY(-2px)` + 阴影加深
- 弹窗/模态框：`0 24px 48px -12px rgba(0,0,0,0.18)`
- 盯办红色任务：附加 `box-shadow: 0 0 0 4px rgba(220,38,38,0.2)` 脉冲动画

### 3.5 动效规范

- 所有交互过渡：`transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1)`
- 任务归档消失：`opacity 0 → 1` + `scale 0.95 → 1` + `translateY(20px → 0)`，持续时间 400ms
...
| `/workbench`                   | 个人工作台 | 已登录      | 核心页面，任务墙布局                  |
| `/workbench/archive`           | 归档查询   | 已登录      | 按标签/人员/时间检索历史任务          |
...
### 5.1 StickyNoteCard（任务卡片组件）

- **Props**：`note`（任务对象）、`mode`（desktop/web）、`permissions`（当前用户权限）
- **状态视觉**：
...
- **State**：`activeNotes`（当前活跃任务列表）、`archivedNotes`（归档列表，分页）、`currentNote`（详情）、`loading`
- **Actions**：
  - `fetchNotes(filters)`：按权限获取任务列表
  - `createNote(payload)`：创建任务，成功后本地prepend
...
- **Getters**：`isAdmin`、`isDeptAdmin`、`canCreateForOthers`（是否可为他人创建任务）
...
1. **创建权限**：
   - 普通用户：仅能创建 `source_type = self` 的任务，owner_id 为自己
   - 部门管理员：可创建 `source_type = assigned` 的任务，owner_id 指定为本部门人员
   - 系统管理员：可创建给任何人，跨部门指派
...
3. **完成与归档**：
   ...
   - 归档后，桌面端对应任务同步消失（通过WebSocket或轮询同步）

4. **盯办预警**：
   - 发起人/组长在Web端点击"盯办" → 调用 `POST /api/v1/notes/{id}/remind`
   - 被盯办人桌面端任务立即变红（WebSocket推送），Web端卡片同步变红
...
| ------------ | ------ | ----------------------------- | ------------------------------------------------------------------------------------------- | ---------------------- | ----------------------------------------------------------------------- |
| 获取任务列表 | GET    | `/api/v1/notes`               | `?status=active&tag_id=&dept_id=&owner_id=&keyword=&page=1&page_size=20`                    | `{data:[],total,page}` | 根据当前路由/标签页自动附加过滤条件；首次加载骨架屏，后续增量更新       |
| 创建任务     | POST   | `/api/v1/notes`               | `{title,content,tags:[],source_type,due_time,owner_id,template_type,group_id,assignees:[]}` | `{data:Note}`          | 成功后本地插入列表顶部；若owner_id≠自己，调用推送接口通知服务端下发桌面 |
| 获取任务详情 | GET    | `/api/v1/notes/{id}`          | -                                                                                           | `{data:Note}`          | 右侧滑出详情面板，富文本渲染                                            |
| 更新任务     | PUT    | `/api/v1/notes/{id}`          | `{title,content,tags,status,due_time}`                                                      | `{data:Note}`          | 乐观更新：先改本地状态，后调接口，失败回滚                              |
| 完成任务     | POST   | `/api/v1/notes/{id}/complete` | `{feedback_content,attachments:[]}`                                                         | `{data:Note}`          | 触发归档动画，400ms后从活跃列表移除；若处于协同模式，通知所有参与者     |
...
| 恢复任务     | POST   | `/api/v1/notes/{id}/restore`  | -                                                                                           | `{data:Note}`          | 从归档恢复至活跃列表，插入顶部                                          |

#### 7.2.2 标签管理接口

| 接口       | 方法 | 路径                | 请求参数                      | 响应数据                                  | 前端处理逻辑                 |
| ---------- | ---- | ------------------- | ----------------------------- | ----------------------------------------- | ---------------------------- |
| 获取标签库 | GET  | `/api/v1/tags`      | `?scope=personal/system/all`  | `{data:[{id,name,color,scope,category}]}` | 创建任务时预加载，本地缓存   |
| 创建标签   | POST | `/api/v1/tags`      | `{name,color,category,scope}` | `{data:Tag}`                              | 即时加入本地标签池，无需刷新 |
| 更新标签   | PUT  | `/api/v1/tags/{id}` | `{name,color}`                | `{data:Tag}`                              | 同步更新所有任务上的标签展示 |
...
| 接口           | 方法    | 路径                          | 请求参数                                  | 响应数据                           | 前端处理逻辑                          |
| -------------- | ------- | ----------------------------- | ----------------------------------------- | ---------------------------------- | ------------------------------------- |
| 获取模板列表   | GET     | `/api/v1/templates`           | `?type=`                                  | `{data:[{id,name,fields,layout}]}` | 创建任务时弹窗选择                    |
...
| ------------------ | ---------- | ------------- | ----------- | ----------------- | ---------------------------- |
| 创建任务（给自己） | ✓          | ✓             | ✓           | ✓                 | 基础功能                     |
| 创建任务（给他人） | ✓          | ✓（限本部门） | ✗           | ✗                 | 需 `can_assign` 权限位       |
| 编辑他人任务       | ✓          | ✓（限本部门） | ✓（限本组） | ✗                 | 仅任务创建者/上级/组长可编辑 |
| 删除任务           | ✓          | ✓（限本部门） | ✗           | ✗（仅自己创建的） | 软删除，管理员可恢复         |
| 盯办操作           | ✓          | ✓             | ✓           | ✗                 | 仅发起人、组长、管理员       |
...
- **夜间模式**：大屏默认深色底（`#0F172A`），与Web端管理端白底区分，任务卡片反色适配
