...
<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <h2 class="text-lg font-semibold text-slate-900 dark:text-slate-100">部门库管理</h2>
    ...
    <div class="grid grid-cols-2 gap-6">
      ...
      <div class="bg-white dark:bg-slate-800 rounded-card border border-slate-100 dark:border-slate-700 p-4 transition-colors duration-300">
        <h4 class="text-sm font-semibold text-slate-700 dark:text-slate-200 mb-3">组织架构</h4>
        <div v-if="loading" class="skeleton h-40 rounded-lg" />
        <div v-else-if="loadError" class="text-sm text-red-400 dark:text-red-400 py-8 text-center">
          {{ loadError }}
          <button class="block mx-auto mt-2 text-xs text-blue-500 dark:text-blue-400 hover:underline" @click="loadData">重试</button>
        </div>
        <div v-else class="space-y-1">
          <template v-for="dept in departments" :key="dept.id">
            <div class="flex items-center gap-1" :data-dept-id="dept.id">
              ...
              >
                <svg :class="['w-3.5 h-3.5 text-slate-400 dark:text-slate-400 transition-transform shrink-0', expanded.has(dept.id) ? 'rotate-90' : '']" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                </svg>
                <span class="flex-1 truncate">{{ dept.name }}</span>
                <span class="text-xs text-slate-400 dark:text-slate-400 shrink-0">{{ dept.member_count }}人</span>
              </button>
              <button
                class="shrink-0 w-6 h-6 flex items-center justify-center rounded text-xs text-slate-300 dark:text-slate-400 hover:text-blue-500 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/40 transition-smooth"
                title="添加子部门"
            ...
            <div v-if="expanded.has(dept.id) && dept.children" class="ml-6 space-y-1 border-l border-slate-100 dark:border-slate-700 pl-4">
              <template v-for="child in dept.children" :key="child.id">
                <div class="flex items-center gap-1">
                  <button
                    class="flex-1 flex items-center gap-2 px-3 py-2 rounded-btn text-sm text-left transition-smooth hover:bg-slate-50 dark:hover:bg-slate-700/50"
                    :class="{ 'bg-blue-50 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300 font-medium': selectedDept?.id === child.id }"
                    @click="selectDept(child)"
                  >
                    <span class="flex-1 truncate">{{ child.name }}</span>
                    <span class="text-xs text-slate-400 dark:text-slate-400 shrink-0">{{ child.member_count }}人</span>
                  </button>
                  <button
                    class="shrink-0 w-6 h-6 flex items-center justify-center rounded text-xs text-slate-300 dark:text-slate-400 hover:text-blue-500 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/40 transition-smooth"
                    title="添加子部门"
      ...
      <!-- 右侧：部门详情 / 编辑 -->
      <div class="bg-white dark:bg-slate-800 rounded-card border border-slate-100 dark:border-slate-700 p-4 transition-colors duration-300">
        <h4 class="text-sm font-semibold text-slate-700 dark:text-slate-200 mb-3">部门详情</h4>
        <div v-if="!selectedDept" class="text-sm text-slate-400 dark:text-slate-500 py-8 text-center">
          请在左侧选择一个部门
        </div>
        <div v-else class="space-y-3">
          <div>
            <span class="text-xs text-slate-400 dark:text-slate-400 mb-1 block">部门名称</span>
            <input v-model="editName" class="input-field !py-1.5 !text-sm" placeholder="部门名称" />
          </div>
          <div>
            <span class="text-xs text-slate-400 dark:text-slate-400 mb-1 block">上级部门</span>
            <p class="text-sm text-slate-700 dark:text-slate-300">{{ getParentName(selectedDept.parent_id || '') }}</p>
          </div>
          <div class="flex items-center gap-2">
            <span class="text-xs text-slate-400 dark:text-slate-400 w-16">级别</span>
            <span class="text-sm text-slate-700 dark:text-slate-300">{{ getDeptLevelLabel(selectedDept.level) || '一级' }}</span>
          </div>
          <div class="flex items-center gap-2">
            <span class="text-xs text-slate-400 dark:text-slate-400 w-16">人数</span>
            <span class="text-sm text-slate-700 dark:text-slate-300">{{ selectedDept.member_count }} 人</span>
          </div>
          ...
          <div class="flex gap-2 pt-3 border-t border-slate-100 dark:border-slate-700">
            ...
            <button class="px-3 py-1.5 text-xs bg-red-50 dark:bg-red-900/40 text-red-600 dark:text-red-300 rounded-btn hover:bg-red-100 dark:hover:bg-red-900/60 transition-smooth disabled:opacity-50" :disabled="deleting" @click="handleDelete">
              {{ deleting ? '删除中...' : '删除部门' }}
            </button>
          </div>
        </div>
    ...
    <Teleport to="body">
      ...
      <div v-if="showNewModal" class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative z-50 bg-white dark:bg-slate-800 rounded-card shadow-modal w-full max-w-sm mx-4 p-6 animate-fade-in">
          <h3 class="text-base font-semibold text-slate-900 dark:text-slate-100 mb-4">新建部门</h3>
          <form @submit.prevent="handleCreate" class="space-y-4">
            <div>
              <span class="text-xs text-slate-500 dark:text-slate-400 mb-1 block">部门名称</span>
              <input v-model="newName" class="input-field" placeholder="请输入部门名称" autofocus />
            </div>
            <div>
              <span class="text-xs text-slate-500 dark:text-slate-400 mb-1 block">上级部门</span>
